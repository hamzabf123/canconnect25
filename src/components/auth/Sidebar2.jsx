...
console.log('Active page:', activePage);
  ...
  const menuItems = [
    ...
    {
      name: 'Recovery',
      icon:  'M12 2C6.477 2 2 6.477 2 12s4.477 10 10 10 10-4.477 10-10S17.523 2 12 2zm0 18c-4.418 0-8-3.582-8-8s3.582-8 8-8 8 3.582 8 8-3.582 8-8 8zm-1-13h2v6h-2zm0 8h2v2h-2z',
      path: '/recovery'
    },
    {
      name: 'Urgence',
            ...
            <li key={item.name} className={activePage === item.name.toLowerCase() ? 'active' : ''}>
              <button onClick={() => handleNavigation(item.path)}>
                <svg 
                  xmlns="http://www.w3.org/2000/svg" 
                  height="24px" 
                  viewBox={['homepage','Dashboard','Recovery','Urgence','Profile'].includes(item.name) ? "0 0 24 24" : "0 -960 960 960"}
                  width="24px" 
                  fill="#1a4b8e"
                >
                  <path d={item.icon} />
